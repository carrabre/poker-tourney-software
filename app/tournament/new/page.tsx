  ...
  const handleCreateTournament = async () => {
    ...
    try {
      ...
      const tournament: Tournament = {
        ...
        buyIn,
        entryFee,
        maxRebuys: 2, // Default value
        rebuyAmount: buyIn, // Default same as buy-in
        rebuyChips: Math.floor(startingChips / 2), // Default half the starting chips
        allowAddOns: true, // Default value
        addOnAmount: buyIn, // Default same as buy-in
        addOnChips: Math.floor(startingChips / 2), // Default half the starting chips
        nextBreak: enableBreaks ? breakInterval : 0,
        breakLength: breakDuration,
        created_at: new Date().toISOString()
      };
      
      // Save tournament to localStorage
      const saveSuccess = saveTournament(tournament);
      if (!saveSuccess) {
        alert('Failed to save tournament to local storage');
        return;
      }
      
      // Save blind structure to localStorage
      ...
      console.log('Tournament created:', {
        ...
        startingChips,
        blindLevels,
        breakInterval: enableBreaks ? breakInterval : 0,
        breakDuration,
